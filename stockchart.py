...
from mplfinance.original_flavor import candlestick_ohlc  # Correct import statement for candlestick_ohlc
...
def run(yahoo_symbol, period, interval):
    ...
    def _get_yfinance_data(yahoo_symbol, period, interval):
        print(f'Retrieving {period}/{interval} data for {yahoo_symbol} from yfinance...')
    ...
    chart_data_ohlc_df.loc[:, 'Order'] = np.arange(len(chart_data_df))  # Corrected index assignment
    ...
    ax.set_xticks(range(0, len(chart_data_df), size))  # Adjusted xticks
    ax.set_xticklabels(chart_data_df['Timestamp'][::size].dt.strftime('%Y.%m.%d'))

    current_values = ax.get_yticks()
    ax.set_yticks(current_values)
    ax.set_xlim(0, len(chart_data_df) - 1)  # Adjusted xlim to match data length
    ...
    ax.text(0.02, 0.97, f'{yahoo_symbol} [{interval}]', transform=ax.transAxes, verticalalignment='top',
            bbox=text_interval_props, color='w')
